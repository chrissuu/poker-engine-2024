            ...
            if observation["opp_pip"] - observation["my_pip"] < 0: #There is a bet against you
                ...
                if observation["opp_pip"] - observation["my_pip"] == -1: #you are small blind first action
                    ...
                    if equity - self.equity_needed_against_bet(bet_size, potsize) < 0: #we have less equity than the price (the price to pay is more than our equity)
                        ...
                        difference = equity - self.equity_needed_against_bet(bet_size, potsize)
                        if difference < 0.03: #WE OPT TO CALL
                            ...
                            if difference > 0.1: #equity difference is high 10%
                                return (observation["min_raise"] * 1.4) #3 betting nearly 3x the raise
                            else:
                                return (observation["min_raise"] * 1.1) 
                ...
                if equity - self.equity_needed_against_bet(bet_size, potsize) < 0: #we have less equity than the price (the price to pay is more than our equity)
                    ...
                    if difference < 0.03: #WE OPT TO CALL
                        ...
                        if difference > 0.1: #equity difference is high 10%
                            ...
                            return (observation["min_raise"] * 1.1) 
            else: #no bet against you you can either check or fold.
                if equity > 0.3: #if you have at least 30% equity
                    return (potsize*2, False)
                else:
                    return (0, False);
