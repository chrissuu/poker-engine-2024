...
class Player(Bot):
                    ...
                    if equity - self.equity_needed_against_bet(bet_size, potsize-bet_size) < 0: #we have less equity than the price (the price to pay is more than our equity)
                        ...
                        if difference < 0.08: #PREFLOP_BLUFFING_STANDARD if the margin we are down equity by is less than 8%
                            ...
                            if x > 10: #90 percent of the times we will opt to fold
                                ...
                                if (observation["min_raise"] * 1.4 > observation["max_raise"]):
                                    return ((observation["max_raise"]), False)
                                return ((observation["min_raise"] * 1.4), False) #3 betting nearly 3x the raise, 1.4
                        ...
                        if difference < 0.3: #WE OPT TO CALL, how much risk are you willing to take relative to your opp equity, 0.3
                            ...
                            if difference > 0.1: #equity difference is high 10%, how much are you confident relative to opp equity, 0.1
                                if (observation["min_raise"] * 1.4 > observation["max_raise"]):
                                    return ((observation["max_raise"]), False)
                                return ((observation["min_raise"] * 1.4), False) #3 betting nearly 3x the raise, 1.4
                            else:
                ...
                if equity - self.equity_needed_against_bet(bet_size, potsize-bet_size) < 0: #we have less equity than the price (the price to pay is more than our equity)
                    ...
                    if difference < 0.05: #WE OPT TO CALL
                        ...
                        if difference > 0.1: #equity difference is high 10%, how much are you confident relative to opp equity, 0.1
                            if (observation["min_raise"] * 1.4 > observation["max_raise"]):
                                return ((observation["max_raise"]), False)
                            return ((observation["min_raise"] * 1.4), False) #equity difference is high 10% 3x bet
                        else:
                            return ((observation["min_raise"] * 1), False)
    ...
    def get_action(self, observation: dict) -> Action:
        ...
        bool_bet, call = self.bet_or_nah(observation, potsize)
